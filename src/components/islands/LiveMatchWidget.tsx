...
export default function LiveMatchWidget({ approvedEntries = [] }: LiveMatchWidgetProps) {
  ...
  if (liveMatches.length > 0) {
    return (
      <div className="p-5 rounded-2xl bg-gradient-to-br from-wc-card to-wc-dark/10 border border-wc-red/35 backdrop-blur-sm relative overflow-hidden shadow-lg shadow-wc-red/5">
        {/* Decoración de fondo */}
        ...
        {/* Header */}
        <div className="flex justify-between items-center border-b border-wc-border/50 pb-2.5 mb-4 relative z-10">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5 font-sports">
            ...
            {liveMatches.length > 1 ? 'Partidos en Vivo' : 'Partido en Vivo'}
          </h3>
        ...
        {/* Botón Ver Pronósticos Unificado */}
        <div className="mt-4 pt-3.5 border-t border-wc-border/30 relative z-10">
          <a
            href="/dashboard/live"
            className="w-full py-2 px-4 rounded-xl bg-wc-dark hover:bg-wc-card text-wc-gold border border-wc-border hover:text-white transition-all duration-200 text-xs font-bold font-sports tracking-wider uppercase flex items-center justify-center gap-1.5 cursor-pointer"
          >
            <Activity className="w-4 h-4 text-wc-gold" />
            <span>Ver pronósticos en vivo</span>
          </a>
        </div>
      </div>
    );
