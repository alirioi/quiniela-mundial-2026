      ...
      <div className="space-y-3 pt-6 border-t border-wc-border/40">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <label className={`cursor-pointer flex items-center justify-center gap-2 p-3 rounded-xl border ${paymentMethod === 'binance_pay' ? 'border-wc-gold bg-wc-gold/10' : 'border-wc-border bg-wc-dark hover:border-slate-600'} transition-all`}>
            <input type="radio" name="paymentMethod" value="binance_pay" checked={paymentMethod === 'binance_pay'} onChange={(e) => setPaymentMethod(e.target.value)} className="hidden" />
          ...
          <label className={`cursor-pointer flex items-center justify-center gap-2 p-3 rounded-xl border ${paymentMethod === 'pago_movil' ? 'border-wc-gold bg-wc-gold/10' : 'border-wc-border bg-wc-dark hover:border-slate-600'} transition-all`}>
            <input type="radio" name="paymentMethod" value="pago_movil" checked={paymentMethod === 'pago_movil'} onChange={(e) => setPaymentMethod(e.target.value)} className="hidden" />
          ...
          <label className={`cursor-pointer flex items-center justify-center gap-2 p-3 rounded-xl border ${paymentMethod === 'transferencia_bs' ? 'border-wc-gold bg-wc-gold/10' : 'border-wc-border bg-wc-dark hover:border-slate-600'} transition-all`}>
            <input type="radio" name="paymentMethod" value="transferencia_bs" checked={paymentMethod === 'transferencia_bs'} onChange={(e) => setPaymentMethod(e.target.value)} className="hidden" />
          ...
          <div className="space-y-3">
            ...
            {euroRate && vesAmount ? (
              <div className="text-sm md:text-base text-slate-300 leading-relaxed flex flex-col gap-1.5">
                <div>
                  Tasa Euro BCV actual: <strong className="text-slate-100 text-base">{formatCurrency(euroRate)} Bs</strong>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span>Monto total a transferir:</span>
                  <strong className="text-wc-gold text-lg md:text-xl font-bold">{formatCurrency(vesAmount)} Bs</strong>
                  <button
                    type="button"
                    onClick={() => copyToClipboard(vesAmount.toFixed(2).replace('.', ','), 'ves_amount')}
                  ...
                  >
                    {copiedId === 'ves_amount' ? <Check className="w-4 h-4 text-wc-green" /> : <Copy className="w-4 h-4" />}
              ...
              <div className="space-y-3">
                <div className="p-3 bg-wc-card rounded-lg border border-wc-border/60 text-xs font-mono space-y-1.5 text-slate-200">
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      <button type="button" onClick={() => copyToClipboard('01020441170000346696', 'tb_cuenta_1')} className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold transition-colors" title="Copiar">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-bold select-all">V23719075</span>
                      <button type="button" onClick={() => copyToClipboard('V23719075', 'tb_ci_1')} className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold transition-colors" title="Copiar">
