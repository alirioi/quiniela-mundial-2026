...
export default function MyEntries({ userFullName }: MyEntriesProps) {
  ...
  const handleFileChange = (selectedFile: File) => {
    ...
    if (!validTypes.includes(selectedFile.type)) {
      setFormError(
        "Formato no válido. Solo se permiten imágenes (JPG, PNG, WEBP) o archivos PDF.",
      );
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-sm text-slate-400 font-sports tracking-wider uppercase">
          Cargando tus cupos...
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {entries.map((entry) => {
          const hasPending =
            entry.status === "approved" && entry.pendingPredictions > 0;
          return (
            <div
              key={entry.id}
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-wc-dark border border-wc-border">
                  <div>
                    <div className="text-xs uppercase font-bold text-slate-500 tracking-wider font-sports">
                      Puntuación
                    </div>
                    <div className="text-2xl font-black text-white mt-1 font-sports tracking-wider">
                      {entry.status === "approved" ? entry.total_points : "--"}
                      <span className="text-xs font-normal text-slate-400 ml-1 font-sports tracking-normal">
                        pts
                      </span>
                    </div>
                  </div>
                      ...
                      {entry.status === "approved" ? (
                        hasPending ? (
                          <span className="text-wc-gold flex items-center gap-1">
                            <AlertTriangle
                              className="w-4 h-4 text-wc-gold"
                              strokeWidth={2.5}
                            />
                          ...
                          <span className="text-wc-green flex items-center gap-1">
                            <CheckCircle2
                              className="w-4 h-4 text-wc-green"
                              strokeWidth={2.5}
                            />
                            <span>Completos</span>
                          </span>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-wc-border/50">
                {entry.status === "approved" && (
                  <a
                    href={
                      entry.activePhases.length > 0
                        ? `/predictions/${entry.activePhases[0].slug}`
                        : "#"
                    }
                ...
                {entry.status === "rejected" && (
                  <div className="text-xs text-wc-red font-medium flex items-start gap-1.5">
                    <XCircle
                      className="w-4 h-4 text-wc-red shrink-0 mt-0.5"
                      strokeWidth={2.5}
                    />
                    <span>
                      El administrador rechazó el comprobante. Por favor, sube
                      un cupo nuevo con un pago válido o contáctanos.
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="p-4 rounded-xl bg-wc-dark border border-wc-border space-y-3">
              ...
              <div className="pt-2 border-t border-wc-border/50 text-xs md:text-sm text-wc-gold font-bold text-center flex items-center justify-center gap-1.5 font-sports tracking-wider uppercase">
                <Megaphone
                  className="w-4 h-4 text-wc-gold shrink-0 animate-pulse-subtle"
                  strokeWidth={2.5}
                />
                <span>
                  El 100% del pote acumulado se entregará al primer lugar (único
                  ganador) al finalizar el torneo.
                </span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-wc-dark/65 border border-wc-border space-y-3 text-slate-300 mt-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            copyToClipboard("139030711", "binance_id")
                          }
              ...
              {(paymentMethod === "pago_movil" ||
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-wc-gold/10 border border-wc-gold/20 rounded-lg text-xs md:text-sm text-wc-gold font-medium leading-relaxed">
                    ...
                    Bolívares, el costo del cupo es de <strong>26 USD</strong>{" "}
                    calculados a la tasa del Euro oficial (BCV) del día del
                    pago. Ese dinero luego se cambiará a USDT y se aplicará la
                    misma distribución de 15 USDT al pote y 5 USDT a gastos
                    operativos.
                  </div>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              copyToClipboard("V23719075", "pm_ci")
                            }
                            className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold transition-colors"
                            title="Copiar"
                          >
                      ...
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              copyToClipboard("04147535965", "pm_tlf")
                            }
                            className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold transition-colors"
                            title="Copiar"
                          >
                        ...
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold"
                              title="Copiar"
                            >
                        ...
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                copyToClipboard("V23719075", "tb_ci_1")
                              }
                              className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold"
                              title="Copiar"
                            >
                        ...
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                copyToClipboard("V23719075", "tb_ci_2")
                              }
                              className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-wc-gold transition-colors"
                              title="Copiar"
                            >
              ...
              <p className="text-xs md:text-sm font-semibold leading-relaxed mt-2 p-2.5 rounded-lg disclaimer-box">
                * IMPORTANTE: No incluyas palabras como "apuesta", "quiniela",
                "sorteo" ni referencias a juegos de azar en la nota de
                transferencia para evitar el bloqueo de las cuentas.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                <label
                  htmlFor="displayName"
                  className="block text-xs md:text-sm font-semibold uppercase tracking-wider text-slate-400 font-sports"
                >
                  Nombre del Cupo
                </label>
                <input
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder={
                    paymentMethod === "binance_pay"
                      ? "Ej: alias@gmail.com o ID 123456"
                  ...
                  {!file ? (
                    <div className="space-y-2">
                      <div className="mx-auto w-10 h-10 rounded-full bg-wc-dark border border-wc-border flex items-center justify-center text-wc-gold">
                        <UploadCloud
                          className="w-5 h-5 text-wc-gold"
                          strokeWidth={2.5}
                        />
                      </div>
                      <div>
                      ...
                      <div className="flex items-center space-x-3 text-left min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-xs font-semibold bg-wc-green/10 text-wc-green mt-1 border border-wc-green/15 font-sports tracking-wider uppercase">
                            <Check
                              className="w-3 h-3 text-wc-green"
                              strokeWidth={2.5}
                            />{" "}
                            Listo
                          </span>
              ...
              {formError && (
                <div className="p-3 rounded-xl bg-wc-red/10 border border-wc-red/20 text-wc-red text-xs font-semibold flex items-center gap-1.5">
                  <AlertTriangle
                    className="w-4.5 h-4.5 text-wc-red shrink-0"
                    strokeWidth={2.5}
                  />
                  <span>{formError}</span>
                </div>
